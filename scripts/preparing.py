...
def preparing(config_file):
    ...
    logger.info("End creating routes")
    ...
    logger.info("Trips divided in trips between control stations.")
    ...
    df_final.to_csv(processed_path / "df_final_for_ml.csv", index=False)
    routes.to_csv(processed_path / 'routes.csv', index=False)
    logger.info(f"df_final_for_ml: {df_final.shape}")
    logger.info("\n")
